...
for measure, stat_idx in measures.items():
    ...
    for line in lines:
        ...
        diff = stat - expected  # todo: divide by SD?
    ...
    with open(measure + '_subresults.txt', 'w') as sub, \
        ...
        for stim, ias in stim_mapi.items():
            ...
            for i, (ia, scores) in enumerate(ias.items()):
                ...
                # if there is non-zero data on a stop word, move to nearest non-stop.
                # if there is tie for nearest non-stop, move to right.

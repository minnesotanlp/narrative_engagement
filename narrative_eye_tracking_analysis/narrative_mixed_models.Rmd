...
all_data_continuous$character_count <- normalize_col(all_data_continuous$word_length * all_data_continuous$word)
...
```{r}
model_highlight_proportion_el = lmer(proportion_x ~ 1 +  character_count + word_freq_avg + positive + negative + concreteness  + valence_diff + arousal_diff + (1 | RECORDING_SESSION_LABEL) , data=el_story)
model_highlight_proportion_sm = lmer(proportion_x ~ 1 +  character_count + word_freq_avg + positive + negative + concreteness  + valence_diff + arousal_diff + (1 | RECORDING_SESSION_LABEL) , data=sm_story)
model_highlight_proportion = lmer(proportion_x ~ 1 + character_count + word_freq_avg + positive + negative + concreteness  + valence_avg_x + arousal_avg_x + valence_diff + arousal_diff + (1 | RECORDING_SESSION_LABEL) + (1 | story), data=all_data_continuous)


model_dwell_time = lmer(norm_dt ~ 1   +  word_freq_avg  + positive + negative + concreteness + valence_avg_x + arousal_avg_x +valence_diff  + arousal_diff + proportion_x + (1 | RECORDING_SESSION_LABEL) + (1 | story), data=all_data_continuous)
model_dwell_time_el = lmer(norm_dt ~ 1   +  word_freq_avg + positive   + negative + concreteness + valence_avg_x + arousal_avg_x +valence_diff  + arousal_diff + proportion_x + (1 | RECORDING_SESSION_LABEL) , data=el_story)
model_dwell_time_sm = lmer(norm_dt ~ 1   +  word_freq_avg  +positive + negative + concreteness + valence_avg_x + arousal_avg_x +valence_diff  + arousal_diff + proportion_x + (1 | RECORDING_SESSION_LABEL) , data=sm_story)
```
...
```{r}
library(ggplot2)
ggplot(all_data_continuous ,aes(x=valence_diff, y=proportion_x, color=story)) + geom_point(alpha = .5) + facet_wrap(~RECORDING_SESSION_LABEL, nrow = 4, ncol = 6)
